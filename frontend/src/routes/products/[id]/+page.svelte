<script lang="ts">
  ...
  let inCart = $derived(selectedSku ? cart.items.find((i) => i.skuId === selectedSku.id) ?? null : null);
              ...
              {#if inCart}
                <button
                  class="w-full h-12 text-sm font-medium tracking-wide uppercase rounded-sm transition-all cursor-pointer
                    border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 active:scale-[0.99]"
                  onclick={() => cart.removeItem(inCart.cartItemId)}
                >
                  Remove from Cart
                </button>
              {:else}
