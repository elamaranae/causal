<script lang="ts">
  ...
  function getThumbnail(item: (typeof cart.items)[number]): string | null {
    const media = item.sku.media?.media;
    if (!media || media.length === 0) return null;
    return media.find((m) => m.primary)?.thumbnail ?? media[0].thumbnail;
  }
                    ...
                    {:else}
                      <ul role="list" class="-my-6 divide-y divide-slate-200">
                        {#each cart.items as item (item.cartItemId)}
                          ...
                          <li class="flex py-6">
                            <div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-slate-200 bg-slate-100">
                              {#if thumbnail}
                                <img src={thumbnail} alt={item.sku.product.name} class="h-full w-full object-cover" />
                              {:else}
                            ...
                            <div class="ml-4 flex flex-1 flex-col">
                              <div>
                                ...
                                {/if}
                                <p class="mt-1 text-sm text-slate-900">${item.sku.price.priceAmount.toFixed(2)}</p>
                              </div>
