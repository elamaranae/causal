												...
												{#each cart.items as item (item.product.id)}
													<li class="flex py-6">
														<div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-slate-200 bg-slate-100">
															{#if item.product.primaryThumbnailUrl}
																<img src={item.product.primaryThumbnailUrl} alt={item.product.name} class="h-full w-full object-cover" />
															{:else}
														...
														<div class="ml-4 flex flex-1 flex-col">
															<div>
																<h3 class="text-base font-medium text-slate-900">{item.product.name}</h3>
															</div>
